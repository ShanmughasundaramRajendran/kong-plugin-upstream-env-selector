...
describe("dynamic-routing (schema)", function()
  ...
  it("defines access_policy and endpoint selector blocks", function()
    ...
    assert.is_table(access_policy.entity_checks)
    assert.is_table(endpoint.entity_checks)

    local access_sni = find_field(access_policy.fields, "sni")
    local endpoint_sni = find_field(endpoint.fields, "sni")
    local access_header = find_field(access_policy.fields, "header_name")
    local access_query = find_field(access_policy.fields, "query_param_name")
    local endpoint_header = find_field(endpoint.fields, "header_name")
    local endpoint_query = find_field(endpoint.fields, "query_param_name")
    ...
    assert.equal(1, access_header.len_min)
    assert.equal(1, access_query.len_min)
    assert.equal(1, endpoint_header.len_min)
    assert.equal(1, endpoint_query.len_min)
  end)
end)
