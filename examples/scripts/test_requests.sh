...
echo "---- dev via X-Upstream-Header ----"
curl "${CURL_OPTS[@]}" -H "X-Upstream-Header: dev" -H "Authorization: Bearer ${TOKEN_NEUTRAL}" "${PROXY}/private/684130/developer-platform/gateway/clients" | head -n 20
echo

echo "---- prod via X-Upstream-Header ----"
curl "${CURL_OPTS[@]}" -H "X-Upstream-Header: prod" -H "Authorization: Bearer ${TOKEN_NEUTRAL}" "${PROXY}/private/684130/developer-platform/gateway/clients" | head -n 20
echo

echo "---- qa via X-Upstream-Header ----"
curl "${CURL_OPTS[@]}" -H "X-Upstream-Header: qa" -H "Authorization: Bearer ${TOKEN_NEUTRAL}" "${PROXY}/private/684130/developer-platform/gateway/clients" | head -n 20
echo
